...
function generateLevel() {
    const size = 8; // Define the size of the level
    const level = Array(size).fill().map(() => Array(size).fill(EMPTY)); // Create an empty level
    ...
    // Function to check if a block position is valid
    function isValidBlockPosition(x, y) {
        if (x <= 1 || x >= size-2 || y <= 1 || y >= size-2) return false; // Ensure position is within bounds
        ...
        if (level[y+1][x] === EMPTY) emptySpaces.vertical++;
        return emptySpaces.horizontal >= 1 && emptySpaces.vertical >= 1; // Ensure there are empty spaces around
    }

    // Function to clear space around a given position
